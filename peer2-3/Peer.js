...
if(messageDecimal == "1"){
    ...
    let connection = net.createConnection(portsArray[1],HOST,()=>{
        connection.on('data', data1 =>{
            ...
            let response1 = data1.toString();
            let v1 = response1.slice(0,3);
            let message1 = response1.slice(3,11);
            let messageDecimal1 = binToDec(message1);
            //console.log(messageDecimal);
            let numbOfPeers1 = response1.slice(11,24);
            
            let senderIdLength1 = response1.slice(24,32);
            let senderID1 = response1.slice(32,32+(38*senderIdLength1.toString(10))).match(/.{1,8}/g).join(' ');
            let peerinformation1 = response1.match(/.{1,8}/g);
            //console.log(peerinformation[9]);
            let senderLength = binToDec(senderIdLength1);
            let peerNumb = binToDec(numbOfPeers1);

    console.log("Connected to peer " +senderIDtext1 +":"+portNumber1+" at timestamp: " + singleton.getTimestamp());
    console.log('This peer address is 127.0.0.1:' + PORT + " located at " + peerName1);
    console.log('Received ack from ' +  senderIDtext1 + ":"+ portNumber1);
    peerConnection.write('Connected from peer 127.0.0.1:'+PORT);
    })
})
}
